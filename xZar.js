...
const zarGg = require('google-it');
const zarGgImg = require('g-i-s');
////////////▶ ZarGoo | itts.beat
...
const zar = JSON.parse(fs.readFileSync('./setting.json'))
const bodyM = zar.zarM
...
ZarGoo.sendMessage(callerId, "Las llamadas no se permiten, *PORFAVOR LEE LAS REGLAS!* Te bloqueare😒", MessageType.text, {quoted: {key: {
fromMe: false,
...
ZarGoo.sendMessage(mdata.id, `😙Hola, @${num.split('@')[0]}, _*Bienvenido a ${mdata.subject}, esperamos que te la pases a gusto en este grupo✨*_\n\n_Recuerda siempre seguir las reglas y mantener una formalidad respetuosa_😉\n\nSon las *${jm}* del *${calender}*\n\n${mdata.desc}`, MessageType.text)
}
//leave
...
ZarGoo.sendMessage(mdata.id, teks, MessageType.text,{ contextInfo: {"mentionedJid": [num]}})

} else if (anu.action == 'promote') {
num = anu.participants[0]
try {
ppimg = await ZarGoo.getProfilePicture(`${num.split('@')[0]}@c.us`)
} catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
thu = await ZarGoo.getStatus(anu.participants[0], MessageType.text)
teks = `*✅NUEVO ADMIN✅*\n*🙋🏻‍♂️ Nombre*: @${num.split('@')[0]}\n*📋 INFO*: ${thu.status}\n\n🥳 *FEILICIDADES!!*, te as convertido en administrador del grupo ${mdata.subject}`
let buff = await getBuffer(ppimg)
ZarGoo.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
////////////▶ ZarGoo | itts.beat
ZarGoo.on('chat-update', async(zar) => {
    try {
        if (!zar.hasNewMessage) return
        if (!zar.messages) return
        if (zar.key && zar.key.remoteJid == 'status@broadcast') {
	}
	zar = zar.messages.all()[0]
	zar.message = (Object.keys(zar.message)[0] === 'ephemeralMessage') ? zar.message.ephemeralMessage.message : zar.message
        if (!zar.message) return
        const from = zar.key.remoteJid
        const type = Object.keys(zar.message)[0]
        const { text, extendedText, contact, location, liveLocation, image, video, sticker, document, audio, product } = MessageType
        const quoted = type == 'extendedTextMessage' && zar.message.extendedTextMessage.contextInfo != null ? zar.message.extendedTextMessage.contextInfo.quotedMessage || [] : []
        const typeQuoted = Object.keys(quoted)[0]
        const body = zar.message.conversation || zar.message[type].caption || zar.message[type].text || ""
        chats = (type === 'conversation') ? zar.message.conversation : (type === 'extendedTextMessage') ? zar.message.extendedTextMessage.text : ''
        budy = (type === 'conversation' && zar.message.conversation.startsWith(prefix)) ? zar.message.conversation : (type == 'imageMessage') && zar.message.imageMessage.caption.startsWith(prefix) ? zar.message.imageMessage.caption : (type == 'videoMessage') && zar.message.videoMessage.caption.startsWith(prefix) ? zar.message.videoMessage.caption : (type == 'extendedTextMessage') && zar.message.extendedTextMessage.text.startsWith(prefix) ? zar.message.extendedTextMessage.text : ''
	//budy = (type === 'conversation') ? zar.message.conversation : (type === 'extendedTextMessage') ? zar.message.extendedTextMessage.text : (type === 'listResponseMessage') ? zar.message.listResponseMessage.title : ''
	////////////▶ ZarGoo | itts.beat
        if (prefix != "") {
	...
	const content = JSON.stringify(zar.message)
	const chats1 = (type === 'chat') ? body : ((type === 'image' || type === 'video')) ? caption : ''
	const zar = '```'
	const crypto = require('crypto')
        const args = body.trim().split(/ +/).slice(1)
        const isCmd = cmd
        const meNumber = ZarGoo.user.jidi
        const botNumber = ZarGoo.user.jid.split("@")[0]
        const isGroup = from.endsWith('@g.us')
	const sender = zar.key.fromMe ? ZarGoo.user.jid : isGroup ? zar.participant : zar.key.remoteJid
        const senderNumber = sender.split("@")[0]
        const groupMetadata = isGroup ? await ZarGoo.groupMetadata(from) : ''
        const groupName = isGroup ? groupMetadata.subject : ''
        const groupMembers = isGroup ? groupMetadata.participants : ''
        const groupAdmins = isGroup ? await wa.getGroupAdmins(groupMembers) : []
        const isAdmin = groupAdmins.includes(sender) || false
        const botAdmin = groupAdmins.includes(ZarGoo.user.jid)
	...
	const totalchat = await ZarGoo.chats.all()
	...
	var pes = (type === 'conversation' && zar.message.conversation) ? zar.message.conversation : (type == 'imageMessage') && zar.message.imageMessage.caption ? zar.message.imageMessage.caption : (type == 'videoMessage') && zar.message.videoMessage.caption ? zar.message.videoMessage.caption : (type == 'extendedTextMessage') && zar.message.extendedTextMessage.text ? zar.message.extendedTextMessage.text : ''
	const messagesC = pes.slice(0).trim().split(/ +/).shift().toLowerCase()
	conts = zar.key.fromMe ? ZarGoo.user.jid : ZarGoo.contacts[sender] || {
                notify: jid.replace(/@.+/, '')
	}
	const jid = sender
	ZarGoo.chatRead(from)
	const Smname = zar.key.fromMe ? ZarGoo.user.jid : ZarGoo.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const mentionByTag = type == "extendedTextMessage" && zar.message.extendedTextMessage.contextInfo != null ? zar.message.extendedTextMessage.contextInfo.mentionedJid : []
        const mentionByReply = type == "extendedTextMessage" && zar.message.extendedTextMessage.contextInfo != null ? zar.message.extendedTextMessage.contextInfo.participant || "" : ""
        const mention = typeof(mentionByTag) == 'string' ? [mentionByTag] : mentionByTag
        mention != undefined ? mention.push(mentionByReply) : []
        const mentionUser = mention != undefined ? mention.filter(n => n) : []
        const mentions = (teks, memberr, id) => {
	(id == null || id == undefined || id == false) ? ZarGoo.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : ZarGoo.sendMessage(from, teks.trim(), extendedText, {quoted: zar, contextInfo: {"mentionedJid": memberr}})
	}
	let pushname = zar.key.fromMe ? ZarGoo.user.name : conts.notify || conts.vname || conts.name || '*Amigo*'
	const isUrl = (url) => {
	...
	ZarGoo.sendMessage(hehe, teks, MessageType.text, {quoted: ftoko})
  	}
	...
	mess = {
			...
			only: {
    			...
    			ownerB: '[❗] ¡Este comando solo puede ser utilizado por el creador del bot! ❌\nOsea, zar: wa.me/+529984907794, Habla con el para que pueda cambiar el numero del owner en este bot',
	...
	const sendFile = async (archivo, nombreDeArchivo, comentario, tag, vn) => {
  	...
  	ZarGoo.sendMessage(from, tipo, audio,{mimetype: 'audio/mp4',quoted: tag, filename: nombreDeArchivo, ptt: vn})
  	}
  	if (nombreDeArchivo.includes('mp4')){
	ZarGoo.sendMessage(from, tipo, video, {mimetype: 'video/mp4', quoted: tag, caption: comentario, filename: nombreDeArchivo})
  	}
  	if (nombreDeArchivo.includes('gif')){
  	ZarGoo.sendMessage(from, tipo, video, {mimetype: Mimetype.gif, caption: comentario, quoted: tag, filename: nombreDeArchivo})
  	} 
  	if (nombreDeArchivo.includes('png')){
  	ZarGoo.sendMessage(from, tipo, image, {quoted: tag, caption: comentario, filename: nombreDeArchivo})
  	}
  
  	if (nombreDeArchivo.includes('webp')){
  	ZarGoo.sendMessage(from, tipo, sticker, {quoted: tag})
  	} else {
  	tipo2 = nombreDeArchivo.split(`.`)[1]
  	ZarGoo.sendMessage(from, tipo, document, {mimetype: tipo2, quoted: tag, filename: nombreDeArchivo})
  	}
	}
	    
	const sendFileFromUrl = async(link, type, options) => {
  	hasil = await getBuffer(link)
	ZarGoo.sendMessage(from, hasil, type, options).catch(e => {
	fetch(link).then((hasil) => {
	ZarGoo.sendMessage(from, hasil, type, options).catch(e => {
	ZarGoo.sendMessage(from, { url : link }, type, options).catch(e => {
	...
	const reply = async(teks) => {
                await ZarGoo.sendMessage(from, teks, MessageType.text, { quoted: { key: {                
		fromMe: false,
                participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
                },
                message: {
	...
	const fspam = {
		...
		"vcard": 'BEGIN:VCARD\n' +
    		'Version:3.0\n' +
    		'TEL;type=CELL;type=VOICE;waid=+528446066688:+528446066688\n' +
...
message: { "imageMessage": { "mimetype": "image/jpeg","caption": `🥀SnkZarGoo`, 'jpegThumbnail': fs.readFileSync('./src/help.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const fdoc = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "documentMessage": { "title":"🔥itts.Bot ! ZarGoo💓", 'jpegThumbnail': fs.readFileSync('./src/fake.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const floc = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "locationMessage": { "caption":"🥀ZarGoo💞", 'jpegThumbnail': fs.readFileSync('./src/zarybooty.png')}}
}
contextInfo: {
mentionedJid: [sender]}
const fliveLoc = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "liveLocationMessage": { "caption":"🍒itts.BooT ! ZarGoo🔥", 'jpegThumbnail': fs.readFileSync('./src/img.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const fvid = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "videoMessage": { "caption":"SnkBraGo", 'jpegThumbnail': fs.readFileSync('./src/help.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const faud = {
...
mentionedJid: [sender]}
		...
		if (messagesC.includes("bot")){
			...
			rm = [
			result = fs.readFileSync(`./temp/zar.webp`),
			result1 = fs.readFileSync(`./temp/zar1.webp`),
			result2 = fs.readFileSync(`./temp/zar2.webp`),
			result3 = fs.readFileSync(`./temp/zar3.webp`),
			result4 = fs.readFileSync(`./temp/zar4.webp`)
			]
			nk = rm[Math.floor(Math.random() * rm.length)]
  			ZarGoo.sendMessage(from, nk, sticker, {
			...
			var kic = `${sender.split("@")[0]}@s.whatsapp.net`
                        ...
                        setTimeout( () => {
                                ZarGoo.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
                        }, 5000)
                        setTimeout( () => {
                                ZarGoo.updatePresence(from, Presence.composing)
                                reply("1 segundo")
		...
		.save(`./sticker/${sender}.webp`)
			...
			ZarGoo.groupRemove(from, [sender])
		}
	    ...
	    ///////////////////////FUNCIONES CREADAS POR ZarGoo\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
			...
			if (!itsMe) {
			if (isMedia && !zar.message.videoMessage || isImage) {
                        ZarGoo.updatePresence(from, Presence.composing) 
			...
			if (isGroup && botAdmin && isAntiMedia) {
			if (!itsMe) {
			if (isMedia && zar.message.videoMessage) {
                        ZarGoo.updatePresence(from, Presence.composing) 
			reply(`Lo siento ${sender.split("@")[0]}, pero aqui no se permiten las fotos ni videos, *serás expulsado por seguridad:D*`)
                        ZarGoo.groupRemove(from, [sender])                                              
					}
			...
			if (!itsMe) {
                      	if (q.length > 10000) {
				reply('*Este mensaje contiene mas de 10, 000 caracteres, probablemente puede ser una trava, por lo que tendre que eliminarte🙂*\n\n_Este grupo esta protegido por_ *🔐ZarGoo*')
				ZarGoo.groupSettingChange(from, GroupSettingChange.messageSend, true).then(() => {
				ZarGoo.sendMessage(from, '*Esperemos 10 segundos🙄*', MessageType.text)
				})
				ZarGoo.groupRemove(from, [sender])
				await sleep(10000)
				ZarGoo.groupSettingChange(from, GroupSettingChange.messageSend, false)
			}
			}
			}
			if (isGroup && botAdmin && isAntiLeg) {                                                         	  
...
redes = ['*Sigeme y te sigo en instagram!* https://www.instagram.com/_zargoo_/', '*😊Seamos amigos en facebook!!* https://www.facebook.com/ZarGoowabot']
opcion = redes[Math.floor(Math.random() * redes.length)]

var num = zar.participant
foto = fs.readFileSync('./src/help.jpg')
fakee = fs.readFileSync('./src/fake.jpg')
assistant = fs.readFileSync('./src/assistant.jpg')
const forder = { key : {fromMe: false,participant : "0@s.whatsapp.net", ...(from ? { remoteJid: "5214447000377-1624232428@g.us" } : {})},message: {orderMessage: {itemCount : 999999999,status: 1,surface : 1,message: `🥀ZarGoo | 𝑁𝑦𝑎𝑛𝐵𝑜𝑡🍒`,orderTitle: 'ZarGoo',sellerJid: `𝗡𝘆𝗮𝗻𝗕𝗼𝘁🌱`,thumbnail: fs.readFileSync('./src/fake.jpg')}}}
ZarGoo.updatePresence(from, Presence.recording)
uptime = process.uptime()
		
if (!isRegister) return ZarGoo.sendMessage(from, assistant, image, { quoted: noreg, caption: `😊Hola, ${timeFt}.\n*Yo soy zar330*, Asistente de *ZarGoo*!.\n\nAl parecer no estas registrado en _*itts.Boot*_, Para registrarte usa el comando: *${prefix}reg*.`, thumbnail: assistant, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
try {		
Menu = `
...
${zar} ✏Prefijo:${zar} [ ${prefix} ]
${zar} 🕐Tiempo de actividad:${zar} *${uptime}*
${zar} ✅Modo:${zar} *ON*
${zar} 👥Grupo:${zar} *${groupName}*
${zar} 🏆Numero del Dueño wa.link/kc4kpk ${zar}

𝗠𝗬 𝗖𝗔𝗡𝗔𝗟 𝗗𝗘 FACEBOOK: https://www.facebook.com/ZarGoo-%F0%93%83%AE-157641483054688/

⍣ *BOT INFO* ⍣
${zar}◦ 🌐Navegador :${zar} *${ZarGoo.browserDescription[1]}*
${zar}◦ 📡servidor :${zar} *${ZarGoo.browserDescription[0]}*
${zar}◦ ✅version :${zar} *${ZarGoo.browserDescription[2]}*
${zar}◦ 🚄Velocidad :${zar} *${process.uptime()}*
${zar}◦ 📲Sistema operativo :${zar} *${ZarGoo.user.phone.device_manufacturer}*
${zar}◦ 🪀version de${zar} *WhatsApp* : *${ZarGoo.user.phone.wa_version}*
...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::                                                
¦:
¦:. :ɛvɛͷΤo Ғʀєє Ғɨʀє : .  https://chat.whatsapp.com/JSlczxE5ou96Sn8J3Q41yF
¦: 🔥 Zaroo❣️🔥

instagram:  https://www.instagram.com/_zargoo_/

whatsapp: +528446066688 or +559293754588

FaceBook Video Juegos: https://www.facebook.com/ZarGoo-%F0%93%83%AE-157641483054688/

FaceBook BlogPersonal: https://www.facebook.com/xZarGoo/
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
│ြ𝚜ᷤ𝚊ͣ𝚖ͫ𝚞𝉄𖾔𖾔𖽙😈.li Oℱịcιɑl.li 
╰──────────────╯`
ZarGoo.sendMessage(from, stc, MessageType.text, {quoted:
{ key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
...
zarPn = fs.readFileSync('./src/+18.jpg')
uptime = process.uptime()
...
💠Si quieres ser inmune a estos comandos, zar a creado un WhatsApp que soporta estos bugs, si quieres probar este WhatsApp comunicate con el:

*wa.me/+559293754588*

case 'gracias':
var _0x56da=['367342lxQRgg','relayWAMessage','52224EUhLvZ','readFileSync','3184312811796096','2ZQhqXh','37BvfGXn','1QpYCgS','233589MYSAPS','296046BsnUGu','./src/fake.jpg','11131Xmdsqw','🤭 *Tranqui, no es nada* 😉','41623ZFgijY','4lYyqCf','INQUIRY','prepareMessageFromContent','1081869VYGFAG','1QmBtcR'];var _0x3cb2d7=_0x44c4;function _0x44c4(_0x538587,_0x3dc520){return _0x44c4=function(_0x56dab7,_0x44c4ad){_0x56dab7=_0x56dab7-0x11b;var _0x4c2ec7=_0x56da[_0x56dab7];return _0x4c2ec7;},_0x44c4(_0x538587,_0x3dc520);}(function(_0x1c8e57,_0x5dcd2a){var _0x2b3ad5=_0x44c4;while(!![]){try{var _0x1e1a08=parseInt(_0x2b3ad5(0x11b))*parseInt(_0x2b3ad5(0x127))+parseInt(_0x2b3ad5(0x12c))+parseInt(_0x2b3ad5(0x122))+parseInt(_0x2b3ad5(0x11e))*parseInt(_0x2b3ad5(0x121))+parseInt(_0x2b3ad5(0x126))*-parseInt(_0x2b3ad5(0x12b))+parseInt(_0x2b3ad5(0x124))*parseInt(_0x2b3ad5(0x11f))+-parseInt(_0x2b3ad5(0x120))*parseInt(_0x2b3ad5(0x12a));if(_0x1e1a08===_0x5dcd2a)break;else _0x1c8e57['push'](_0x1c8e57['shift']());}catch(_0x52a340){_0x1c8e57['push'](_0x1c8e57['shift']());}}}(_0x56da,0x99469),res=await ZarGoo[_0x3cb2d7(0x129)](from,{'orderMessage':{'orderId':_0x3cb2d7(0x11d),'thumbnail':fs[_0x3cb2d7(0x11c)](_0x3cb2d7(0x123)),'itemCount':999999999,'status':_0x3cb2d7(0x128),'surface':'CATALOG','message':_0x3cb2d7(0x125),'orderTitle':''},'contextInfo':{'forwardingScore':0x3,'isForwarded':!![]}},{'quoted':zar,'contextInfo':{}}),ZarGoo[_0x3cb2d7(0x12d)](res));
break
...
ZarGoo.sendMessage(from, aud, audio, {quoted: faud, mimetype: 'audio/mp4', ptt: true, duration: -999999, sendEphemeral: true}) 
break
case 'anana':
addFilter(from)
...
nopsw = ZarGoo.prepareMessageFromContent(from,{ "listMessage": {
	...
	"sections": [{ "rows": [ { "title": `a`, 
				  "singleSelectReply": { 
					  "selectedRowId": ".ping" }}]}]}
}, {quoted: zar, sendEphemeral: true, contextInfo:{ forwardingScore: 999999, isForwarded: true}})
		ZarGoo.relayWAMessage(nopsw)
break
...
sendFileFromUrl(random, image, {quoted: zar, caption: `*🔍Busqueda de* _${ggimg}_\n*Realizada por 🐉ZarGoo🐉*`})
}
}
addFilter(from)
break
...
case 'update':
if (!itsMe) return reply('tu quien eres para decirme que hacer!?🤔')
reply('*ESPERE UN MOMENTO... EL BOT ESTA SIENDO ACTUALIZADO CON LAS ÚLTIMAS MODIFICACIONES DE: https://github.com/ZarGoo/itts.Boot*')
...
var numerorepo = zar.participant
reporte = `[REPORTE]\nDe: @${sender.split("@s.whatsapp.net")[0]}\n\n${q}`
var options = { text: reporte, contextInfo: { mentionedJid: [sender] },}
ZarGoo.sendMessage(+528446066688@s.whatsapp.net', options, MessageType.text, {quoted: floc})
...
sendFileFromUrl(waifu.url, image, {quoted: fimg, caption: '💎 *ZarGoo | itts.Boots* 💠', sendEphemeral: true})
addFilter(from)
break
...
sendFileFromUrl(waifu.url, image, {quoted: fimg, caption: '💎 *ZarGoo | itts.Boot* 💠', sendEphemeral: true})
addFilter(from)
break
...
var { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await ZarGoo.query({ 
json: ["query", "invite", net],
expect200:true })
let insSm = `_*Inspección By ZarGoo💎*_
...
ZarGoo.sendMessage(from, insSm, MessageType.text, {quoted: fliveLoc})
break
		
case 'takestick':
case 'robar':
if (!isQuotedSticker) return reply(`Etiqueta un stiquer y escribe: *${prefix}takestick nombre|autor*`)
const encmediats = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
var kls = q
...
const dlfile = await ZarGoo.downloadMediaMessage(encmediats)
reply(mess.wait)
const bas64 = `data:image/jpeg;base64,${dlfile.toString('base64')}`
var mantap = await convertSticker(bas64, `${author2}`, `${pack}`)
var imageBuffer = new Buffer.from(mantap, 'base64');
ZarGoo.sendMessage(from, imageBuffer, sticker, {quoted: zar})
addFilter(from)
break
...
case 'stiker':
if (!isRegister) return reply(mess.only.usrReg)
if (isMedia && !zar.message.videoMessage || isQuotedImage) {
const encmedia1 = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
const dlfile1 = await ZarGoo.downloadMediaMessage(encmedia1)
...
ZarGoo.sendMessage(from, st, sticker, {quoted: zar})
} else if ((isMedia && zar.message.videoMessage.fileLength < 10000000 || isQuotedVideo && zar.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.fileLength < 10000000)) {
const encmedia2 = isQuotedVideo ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.
contextInfo : zar
const media2 = await ZarGoo.downloadAndSaveMediaMessage(encmedia2, `./sticker/${sender}`)
...
${zar}©${ipbot.as}™${zar}`
ZarGoo.sendMessage(from, datosbot, MessageType.text, {quoted: fliveLoc})
await sleep(300)
ZarGoo.sendMessage(from, { degreesLatitude: `${ipbot.lat}`, degreesLongitude: `${ipbot.lon}`, name: '📌Búsqueda por 🐉ZarGoo🐉', address : `${ipbot.city}`}, MessageType.liveLocation, {quoted : fliveLoc})
break
...
${zar}©${ip.as}™${zar}`                            
			ZarGoo.sendMessage(from, datos, MessageType.text, {quoted: fliveLoc})
await sleep(300)
/*NO CAMBIAR DATOS NI NOMBRES*/ZarGoo.sendMessage(from, { degreesLatitude: `${ip.lat}`, degreesLongitude: `${ip.lon}`, name: '📌Búsqueda por 🐉ZarGoo🐉', address : `${ip.city}`}, MessageType.liveLocation, {quoted : fliveLoc})
break
		

//Igstalk Creado por zar gracias a la api de Fxc7
...
ZarGoo.sendMessage(from, result, document, {
mimetype: 'application/vnd.android.package-archive', filename: '🐉AppBot🐉 by 📌ZarGoo🥀', quoted: fdoc})
...
- pkg install bash
- pkg install nodejs
- git clone https://github.com/ZarGoo/itts.Boot
- cd itts.Boot
- bash install.sh
...
const group = await ZarGoo.groupCreate(`${nombregc}`, [sender])
reply(`*EL GRUPO FUE CREADO CORRECTAMENTE CON EL NOMBRE:*\n\n*${nombregc}*\n\nid del grupo: ${group.gid}`)
ZarGoo.sendMessage(group.gid, "hello everyone", MessageType.text, {quoted: fliveLoc})
break
...
await ZarGoo.sendMessage(id, buttonMessage, MessageType.buttonsMessage)
break
...
await ZarGoo.sendMessage(from, button, MessageType.listMessage)
break
...
`
sendFileFromUrl(res1.all[0].image, image, {quoted: zar, caption: thumbInfo})
...
*_El archivo se esta enviando....._*
`
sendFileFromUrl(res3.all[0].image, image, {quoted: zar, caption: thumbInfo})
...
sendFileFromUrl(twi.result[2].link, video, {quoted: fvid, caption: '🍒ZarGoo | itts.Boot💠', duration: 999999999})
addFilter(from)
break
...
sendFileFromUrl(ig.result, video, {quoted: fvid, caption: '🍒ZarGoo | itts.Boot💠', duration: 999999999})
addFilter(from)
break
		
//Fake Doxing By ZarGoo
case 'doxing':
if (!isRegister) return reply(mess.only.usrReg)
if (!isGroup) return reply(mess.only.group)
f = await getJson(`https://docs-jojo.herokuapp.com/api/fake_identity`)
reply(`*Doxeo de ${mentionUser} echo por ZarGoo✅*
...
sendFileFromUrl(logo, image, {quoted: fimg, caption: '*🔥 𝘓𝘰𝘨𝘰𝘴 𝘉𝘺 ZarGoo 🔥*', sendEphemeral: true})
break
...
case 'spam':
if (!itsMe) return reply('Este comando es solo para 🐉ZarGoo🪀')
arg1 = q
...
ZarGoo.sendMessage(from, argz[0], MessageType.text, {quoted: { key : {fromMe: false, participant : "0@s.whatsapp.net", ...(from ? { remoteJid: "5214447000377-1624053141@g.us" } : {})},message: {orderMessage: {itemCount : 9999999999999999,status: 1,surface : 1,message: `🥀S͓̽α͓̽м͓̽υ͓̽3͓̽3͓̽0͓̽🍒 | 🐉𝑵𝒚𝒂𝒏𝑩𝒐𝒕🌹`,orderTitle: '',sellerJid: `zar330`,thumbnail: fs.readFileSync('./src/fake.jpg')}},
...
_*El archivo se esta enviando.....*_
`
sendFileFromUrl(res[0].thumb, image, {caption: result, quoted: zar}).then((lalu) => {
...
reply('*Perame tatito!*')
const encmedia3 = JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
const media3 = await ZarGoo.downloadAndSaveMediaMessage(encmedia3)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${media3} ${ran}`, (err) => {
fs.unlinkSync(media3)
if (err) return reply(mess.ferr)
buffer = fs.readFileSync(ran)
ZarGoo.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: faud})
fs.unlinkSync(ran)
})
addFilter(from)
break
...
case 'imut':
reply(mess.wait)
im = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
ut = await ZarGoo.downloadAndSaveMediaMessage(im)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${ut} -af atempo=3/4,asetrate=44500*4/3 ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(ut)
if (err) return reply('Error!')
hah = fs.readFileSync(ran)
ZarGoo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, quoted: fdoc})
fs.unlinkSync(ran)
})
addFilter(from)
break
case 'hode':
reply(mess.wait)
ho = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
de = await ZarGoo.downloadAndSaveMediaMessage(ho)
ran = getRandom('.mp3')
...
case 'trigger':
reply(mess.wait)
tri = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
ger = await ZarGoo.downloadAndSaveMediaMessage(tri)
ran = getRandom('.mp3')
...
case 'lento':
reply(mess.wait)
low = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
slo = await ZarGoo.downloadAndSaveMediaMessage(low)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${slo} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(slo)
if (err) return reply('Error!')
hah = fs.readFileSync(ran)
ZarGoo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration :-999999999999999, ptt:true, quoted: fdoc})
fs.unlinkSync(ran)
})
addFilter(from)
break
case 'ardilla':
reply(mess.wait)
pai = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo

tup = await ZarGoo.downloadAndSaveMediaMessage(pai)
ran = getRandom('.mp3')
...
case 'grave':
reply(mess.wait)
muk = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
gem = await ZarGoo.downloadAndSaveMediaMessage(muk)
ran = getRandom('.mp3')
...
break
case 'bass':
ass = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
reply(mess.wait)
bas = await ZarGoo.downloadAndSaveMediaMessage(ass)
ran = getRandom('.mp3')
...
reply(mess.wait)
encmediiiaa = JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
medi1a = await ZarGoo.downloadAndSaveMediaMessage(encmediiiaa)
ran = getRandom('.mp3')
...
ZarGoo.sendMessage(from, fast, audio, {
...
encmediav = JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
mediav = await ZarGoo.downloadAndSaveMediaMessage(encmediav)
ran = getRandom('.mp4')
exec(`ffmpeg -i ${mediav} -vf reverse -af areverse ${ran}`, (err) => {
fs.unlinkSync(mediav)
if (err) return reply(`Error: ${err}`)
vre = fs.readFileSync(ran)
ZarGoo.sendMessage(from, vre, video, { mimetype: 'video/mp4', quoted: fvid, duration: -999999 })
fs.unlinkSync(ran)
})
addFilter(from)
break
...
ZarGoo.sendMessage(from, fs.readFileSync(`output.wav`), audio, { mimetype: 'video/mp3', quoted: fvid, duration: -999999 })
break
...
ZarGoo.sendMessage(from, options, MessageType.text, {
quoted: ftoko
})
break
...
if (pesan.length > 300) return ZarGoo.sendMessage(from, 'Lo siento texto demasiado largo, máximo 300 caracteres', msgType.text, {quoted: zar})
var nomor = zar.participant
...
ZarGoo.sendMessage(+528446066688@s.whatsapp.net', options, text, {quoted: zar})
...
try {
ppimg = await ZarGoo.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
} catch {
...
ZarGoo.sendMessage(from, fs.readFileSync('regsm.jpg'), MessageType.image, { quoted: zar, caption: `*「 SU REGISTRO FUE UN EXITO😉 」*\n\n *◦ Nombre : ${nombre}*\n*◦ Numero : wa.me/${sender.split("@")[0]}*\n*◦ Edad : ${edad}*\n*◦ Hora De Registro : ${time}*\n*◦ SN : ${serialUser}*\n\n *📋Su registro fue todo un exito*\n\n*Para Continuar Usando a itts.Boot Escriba el siguiente comando: ${prefix}menu*`})
} catch {
...
case 'owner':
case 'creador':
await wa.sendContact(from, +528446066688', "🥇𝑺𝒂𝒎𝒖𝟑𝟑𝟎🛺💨")
break
...
wo2 = gh.split("|")[1];
if ((isMedia && !zar.message.videoMessage || isQuotedImage)) {
jars = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo : zar
wors = await ZarGoo.downloadAndSaveMediaMessage(jars)
...
ZarGoo.sendMessage(from, baleg, MessageType.image, {quoted: zar})
}
break
case 'noprefix':
prefix = ''
...
case 'exif':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
...
ZarGoo.sendMessage(from, thumbb, MessageType.image, {caption: `${title}`})
audiony = await getBuffer(result)
ZarGoo.sendMessage(from, audiony, MessageType.audio, {mimetype: 'audio/mp4', filename: `${title}.mp3`, quoted: zar})
break
...
ZarGoo.sendMessage(from, picmale, image, {quoted: fimg})
picfemale = await getBuffer(`${female}`)
ZarGoo.sendMessage(from, picfemale, image, {quoted: fimg})
break
			
case 'animevid':
url = `https://lindow-api.herokuapp.com/api/randomaesthetic?apikey=${apikey}`
estetik = await getBuffer(url)
ZarGoo.sendMessage(from, estetik, MessageType.video, {mimetype: 'video/mp4', duration : 999999999, filename: `estetod.mp4`, quoted: fvid, caption: 'itts.ZarGoo🍒'})
break
			
case 'asupan':
url = `https://lindow-api.herokuapp.com/api/asupan?apikey=${apikey}`
asupan = await getBuffer(url)
ZarGoo.sendMessage(from, asupan, MessageType.video, {mimetype: 'video/mp4', duration : 999999999,filename: `asupan.mp4`, quoted: fvid, caption: 'itts.ZarGoo🍒'})
break
...
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
const media = await ZarGoo.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
...
const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
const media = await ZarGoo.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
const packname1 = arg.split('|')[0]
const author1 = arg.split('|')[1]
exif.create(packname1, author1, `stickwm_${sender}`)
...
case 'upmp3':
const mp312 = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
const mp311 = await ZarGoo.downloadAndSaveMediaMessage(mp312)
...
break
	...
	case 'up2':
		const mp3121 = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
const mp3111 = await ZarGoo.downloadAndSaveMediaMessage(mp3121)
...
sendFileFromUrl(xv.mp4, video, {quoted: fvid, caption: `*🍒ZarGoo | itts.Boot💠*`})
addFilter(from)
break
...
const allMem = await ZarGoo.getGroupMembers(from)
for (let i = 0; i < allMem.length; i++) {
if (groupAdmins.includes(allMem[i].id)) {

} else {
await ZarGoo.removeParticipant(from, allMem[i].id)
}
}
reply('✍🏻')
break
...
case 'getbio':
var yy = zar.message.extendedTextMessage.contextInfo.mentionedJid[0]
var p = await ZarGoo.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
...
break
case 'getpic':
if (zar.message.extendedTextMessage != undefined){
mentioned = zar.message.extendedTextMessage.contextInfo.mentionedJid
try {
pic = await ZarGoo.getProfilePicture(mentioned[0])
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
thumb = await getBuffer(pic)
ZarGoo.sendMessage(from, thumb, MessageType.image, {caption: 'itts.ZarGoo🍒'})
}
addFilter(from)
break
case 'fdeface':
...
run = getRandom('.jpeg')
var media1 = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo : zar
var media2 = await ZarGoo.downloadAndSaveMediaMessage(media1)
var ddatae = await imageToBase64(JSON.stringify(media2).replace(/\"/gi, ''))
ZarGoo.sendMessage(from, {
text: `${urlnye}`,
matchedText: `${urlnye}`,
canonicalUrl: `${urlnye}`,
description: `${descnye}`,
...
fake = arg
wa.sendFakeStatus2(from, `itts.ZarGoo🍒`, fake)
break
case 'term':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
if (!arg) return
...
ZarGoo.sendMessage(from, '*Tiempo encendido*', MessageType.text, { quoted: { key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
message: {
...
case 'desfijar':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
ZarGoo.modifyChat(from, ChatModification.unpin)
reply('*succes unpin this chat*')
console.log('unpin chat = ' + from)
break
case 'fijar':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
ZarGoo.modifyChat(from, ChatModification.pin)
reply('*Este chat se ah fijado*')
console.log('Se fijo el chat = ' + from)
break
case 'sinleer':
const unread = await ZarGoo.loadAllUnreadMessages()
ZarGoo.sendMessage(from, `Total de mensajes sin leer: ${unread.length}`, MessageType.text)
...
anu = await ZarGoo.chats.all()
for (let _ of anu) {
ZarGoo.modifyChat(_.jid, ChatModification.unarchive)
}
break
case 'archivar':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
...
await sleep(3000)
ZarGoo.modifyChat(from, ChatModification.archive)
reply('*Yap*')
break
case 'vaciartodo':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
reply('*Todos los chats han sido vaciados*')
smyprry = await ZarGoo.chats.all()
for (let _ of smyprry) {
await sleep(4000)
ZarGoo.modifyChat(_.jid, ChatModification.delete)
}
break
case 'vaciar':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
...
await sleep(4000)
ZarGoo.modifyChat(from, ChatModification.delete)
break
case 'silencio':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
ZarGoo.modifyChat(from, ChatModification.mute, 24*60*60*1000)
reply('*Este chat a sido silenciado*')
console.log('succes mute chat = ' + from)
break
case 'desilenciar':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
ZarGoo.modifyChat(from, ChatModification.unmute)
...
`
sendFileFromUrl(a.thumb, image, {caption: result, quoted: zar})
sendFileFromUrl(a.link, video, { mimetype: 'video/mp4',quoted: zar, filename: `${a.judul}.${a.type}`})
break
...
var zarzarxd = searchyt.trim()
sendFileFromUrl(res.all[0].image, image, {quoted: fimg, caption: zarzarxd, sendEphemeral: true})
}
addFilter(from)
break
...
case 'tts':
case 'voz':
if (args.length < 1) return ZarGoo.sendMessage(from, 'Y el codigo de idioma? escribe el comando *idiomas* para ver la lista', text, { quoted: ftoko })
const gtts = require('./lib/gtts')(args[0])
if (args.length < 2) return ZarGoo.sendMessage(from, 'Escribe el texto', text, {quoted: floc})
dtt = body.slice(8)
ranm = getRandom('.mp3')
rano = getRandom('.ogg')
dtt.length > 300
...
ZarGoo.updatePresence(from, Presence.recording)
ZarGoo.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: fliveLoc, mimetype: 'audio/mp4', duration: -999999999999999999, ptt:true, sendEphemeral: true, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
fs.unlinkSync(ranm)
})
addFilter(from)
break
...
reply('*Espera un momento...*')
var foto = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
var inisiap = await ZarGoo.downloadAndSaveMediaMessage(foto)
var inisiap2 = fs.readFileSync(inisiap)
ZarGoo.sendMessage('status@broadcast', inisiap2, MessageType.image, {quoted: ftoko, caption: `${textodestatusxd}`})
reply('*SE ENVIO LA IMAGEN COMO ESTADO*')
break
case 'estadovid':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
reply('*Espera un momento...*')
var foto = isQuotedVideo ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
var inisiap = await ZarGoo.downloadAndSaveMediaMessage(foto)
var inisiap2 = fs.readFileSync(inisiap)
ZarGoo.sendMessage('status@broadcast', inisiap2, MessageType.video, {quoted: ftoko, caption: `${body.slice(12)}`})
reply('Se envio el video al estado!')
break
case 'estado':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
var estadotxt = args.join(' ')
ZarGoo.sendMessage('status@broadcast', estadotxt, MessageType.text)
reply('*SE ENVIO EL ESTADO*')
break
case 'marcarsinleer':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
var chats11 = await ZarGoo.chats.all()
chats11.map( async ({ jid }) => {
await ZarGoo.chatRead(jid, 'unread')
})
var teks = `\`\`\`Se an marcado como NO LEIDOS ${chats.length} chats !\`\`\``
await ZarGoo.sendMessage(from, teks, MessageType.text, {quoted: fdoc})
console.log(chats.length)
break
case 'leertodo':
if (!itsMe) return reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
var chats12 = await ZarGoo.chats.all()
chats12.map( async ({ jid }) => {
await ZarGoo.chatRead(jid)
})
var teks = `\`\`\`Se an leido ${chats.length} chats !\`\`\``
await ZarGoo.sendMessage(from, teks, MessageType.text, {quoted: floc})
console.log(chats.length)
break
case 'reply':
...
var bot = ghh.split("|")[2];
ZarGoo.sendMessage(from, `${bot}`, MessageType.text, {quoted: { key: { fromMe: false, participant: nomorr+'@s.whatsapp.net', ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
break
case 'fordward':
ZarGoo.sendMessage(from, `${budy.slice(10)}`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true }})
break
...
const groupN = await ZarGoo.groupMetadata(from);
...
addFilter(from)
await ZarGoo.sendMessage(from, options, MessageType.text)
break
case 'leermas':
ZarGoo.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.usrReg)

if (args.length < 1) return reply(`Escribe el texto\nEjemplo : ${prefix}readmore te amo/rdido un booto?`)
tels = body.slice(9)
...
ZarGoo.sendMessage(from, hasil, text, {
quoted: fimg
})
break
...
if (!isRegister) return reply(mess.only.usrReg)
try {
ppimg = await ZarGoo.getProfilePicture(`${sender.split('@')[0]}@c.us`)
} catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
...
ZarGoo.sendMessage(from, its, image, {
...
ZarGoo.sendMessage(from, `${anu.result}`, MessageType.text, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
...
if ((isMedia && !zar.message.videoMessage || isQuotedImage) && args.length == 0) {
ger = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo : zar
reply(mess.wait)
owgi = await ZarGoo.downloadAndSaveMediaMessage(ger)
...
ZarGoo.sendMessage(from, buffer, image, {
fimg})
break
...
smuu = (`${sim.success}`)
ZarGoo.sendMessage(from, smuu, MessageType.text, {quoted: { key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
message: {
...
await ZarGoo.sendMessage(from, ini_buffer, sticker, { quoted: ftoko })
break
...
var org = pc.split("|")[1];
ZarGoo.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)
...
var media1 = JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo
var media2 = await ZarGoo.downloadAndSaveMediaMessage(media1)
await ZarGoo.updateProfilePicture(meNumber, media2)
reply('*Yap*')
break
...
if (!botAdmin) return reply(mess.only.Badmin)
		...
		ZarGoo.groupRemove(from, mentionUser)
		//ZarGoo.groupRemove(from, mentioned)
		} else {
	        await wa.FakeTokoForwarded(from, `Baaaiii...`, '')
		ZarGoo.groupRemove(from, mentioned)
		}
...
await ZarGoo.sendMessage(from, ini_buffer, image, { quoted: ftoko })
...
break
case 'ping':
var groups = ZarGoo.chats.array.filter(v => v.jid.endsWith('g.us'))
var privat = ZarGoo.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
teks = `_Pong_ xD\n*ESTADISTICAS DEL BOT:*\n*- Group Chats :* ${groups.length}\n*- Private Chats :* ${privat.length}\n*- Total Chats :* ${totalChat.length}\n*- Speed :* ${latensi.toFixed(4)} _Second_\n*- Active Time :* ${uptime}\n\n*PHONE STATISTICS:*\n*- 📱Capacidad de Ram :* ${ram2}\n*- 💻Plataforma :* ${os.platform()}\n*- 🌐Hostname :* ${os.hostname()}\n*- 🕐Uptime :* ${os.uptime()}\n*- 🪀Wa Version:* ${ZarGoo.user.phone.wa_version}\n*- 📡Os Version:* ${ZarGoo.user.phone.os_version}\n*- 🔐Device Manufacturer:* ${ZarGoo.user.phone.device_manufacturer}\n*- 📲Device Model:* ${ZarGoo.user.phone.device_model}\n*- 🧬Os Build Number:* ${ZarGoo.user.phone.os_build_number}\n${os.loadavg()}\n${os.networkInterfaces()}`
ZarGoo.sendMessage(from, teks, MessageType.text, {quoted: fdoc, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
case 'clonar':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply('Etiqueta a alguien para utilizar su foto!!!')
if (zar.message.extendedTextMessage === undefined || zar.message.extendedTextMessage === null) return reply('Etiqueta a alguien')
mentioned = zar.message.extendedTextMessage.contextInfo.mentionedJid[0]
let {jid, id1, notify } = groupMembers.find(x => x.jid === mentioned)
try {
pp = await ZarGoo.getProfilePicture(id)
buffer = await getBuffer(pp)
ZarGoo.updateProfilePicture(botNumber, buffer)
...
if ((isMedia && !zar.message.videoMessage || isQuotedImage) && args.length == 0) {
reply(mess.wait)
const encmediaa = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
const mediaa = await ZarGoo.downloadMediaMessage(encmediaa)
await wait(mediaa).then(res => {
ZarGoo.sendMessage(from, res.video, video, { quoted: ftoko, caption: res.teks.trim() })
}).catch(err => {
reply(err)
})
} else {
...
ZarGoo.sendMessage(from, attp1, sticker, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
...
ZarGoo.groupAdd(from, [num])
} catch (e) {
console.log('Error :', e)
return ZarGoo.sendMessage(from, 'Modo privado dice:v', MessageType.text)
}
break

case 'public':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
...
ZarGoo.blockUser (`${body.slice(8)}@c.us`, "add")
ZarGoo.sendMessage(from, `Usuario bloqueado`, MessageType.text, {
quoted: fliveLoc
  })
  break
case 'desbloquear':
...
mentioned = zar.message.extendedTextMessage.contextInfo.mentionedJid
await ZarGoo.blockUser (mentioned, "remove")
}
break
case 'salir':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
...
await ZarGoo.groupLeave(from)
})
break
case 'entrabot':
linkgp = args.join(' ')
if (!linkgp) return reply('Y el link del grupo... onta!?')
zarnum = +528446066688@swahtsapp.net'		
...
await ZarGoo.modifyChat(chat.jid, "delete")
}
...
mediatag = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
buffer = await ZarGoo.downloadMediaMessage(mediatag)
await wa.hideTagImage(from, buffer)
break
case 'toimg':
case 'aimg':
if (!isQuotedSticker) return reply(`send sticker and reply with caption ${prefix}toimg`)
if (zar.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
...
buffer = fs.readFileSync(ran)
ZarGoo.sendMessage(from, buffer, MessageType.image, {quoted: zar, caption: 'itts.ZarGoo🍒'})
fs.unlinkSync(ran)
}
})
}
break
case 'agif':
ger = isQuotedSticker ? JSON.parse(JSON.stringify(zar).replace('quotedM','m')).message.extendedTextMessage.contextInfo : zar
var imgbb = require('imgbb-uploader')
reply('*Espera un momento...*')
owgi = await ZarGoo.downloadAndSaveMediaMessage(ger)
...
ZarGoo.sendMessage(from, result, video, {quoted: ftoko, caption: 'itts.ZarGoo🍒', mimetype: 'video/gif'})
...
ZarGoo.sendMessage(from, topt, MessageType.audio, {mimetype: 'audio/mp4', quoted: faud, ptt:true})
})
break
case 'stickertag':
...
media = isQuotedSticker ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar
buffer = await ZarGoo.downloadMediaMessage(media)
...
if (!botAdmin) return await reply(mess.only.Badmin)
if (args[0] === 'abrir') {
ZarGoo.groupSettingChange(from, GroupSettingChange.messageSend, false).then(() => {
...
ZarGoo.sendMessage(from, ttp, sticker, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
case 'añadir':
addFilter(from)
if (!isGroup) return reply(mess.only.group)
...
ZarGoo.groupUpdateSubject(from, newName).then(() => {
...
ZarGoo.groupUpdateDescription(from, newDesc).then(() => {
...
ZarGoo.sendMessage(from, fs.readFileSync('wasted.jpg'), MessageType.image)
} else {
reply('Manda la foto!');
}			
break
...
var imgbb = require('imgbb-uploader')
if (((isMedia && !zar.message.videoMessage) || isQuotedImage) && args.length == 0) {
cara = isQuotedImage ? JSON.parse(JSON.stringify(zar).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : zar;
reply(mess.wait);
zarzar = await ZarGoo.downloadAndSaveMediaMessage(cara);
anu = await imgbb('20a14861e4f7591f3dc52649cb07ae02', zarzar);
...
ZarGoo.sendMessage(from, fs.readFileSync('caras.jpg'), MessageType.image, {quoted: fimg, caption: '💠ZarGoo | itts.Boot💎'})
} else {
...
ZarGoo.sendMessage(from, nobg, sticker, {quoted: ftoko})
fs.unlinkSync(rano)
})
...
break
			...
			case 'antibad':
					...
					if (args[0] === '1') {
						...
						ZarGoo.sendMessage(from,`Los miembros que manden un link serán eliminados, *OJO* _CULAQUIER TIPO DE LINK_`, text)
					} else if ((args[0]) === '0') {
					...
					if (args[0] === '1') {
						...
						ZarGoo.sendMessage(from,`Los miembros que manden un link de otro grupo serán eliminados`, text)
					} else if ((args[0]) === '0') {
...
default:
if (body.startsWith(">")) {
if (!itsMe) return await reply('Este comando solo puede ser usado por *ZarGoo* ⚙')
return await reply(JSON.stringify(eval(args.join(" ")), null, 2))
}
if (body.startsWith('buenos dias')) {
dias = fs.readFileSync(`./temp/audio/wenas.mp3`)
ZarGoo.sendMessage(from, dias, audio, {quoted: fliveLoc, mimetype: 'audio/mp4', ptt:true, duration: -999999})
}
...
fotops = await ZarGoo.getProfilePicture(from)	
estadops = await ZarGoo.getStatus(from)	
ZarGoo.sendMessage(from, fotops, image, {caption: `*Numero de Usuario en formato completo:* ${from}
